...
namespace ruzhouxie
{
    ...
    public:
        ...
        RUZHOUXIE_INLINE constexpr auto process_tape(Tape&& tape)const
            noexcept(noexcept(process_tape_Impl<T, Offset>(std::forward<Tape>(tape), std::make_index_sequence<std::tuple_size_v<Tuple>>{})))
            ->decltype(auto)
        ...
        {
            return process_tape_Impl<T, Offset>(std::forward<Tape>(tape), std::make_index_sequence<std::tuple_size_v<Tuple>>{});
        }
            //AS_EXPRESSION(process_tape_Impl<T, Offset>(std::forward<Tape>(tape), std::make_index_sequence<std::tuple_size_v<Tuple>>{}))
    };
